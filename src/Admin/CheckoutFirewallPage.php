...
final class CheckoutFirewallPage {
	...
	public function assets( string $hook ): void {
		...
		wp_enqueue_script( 'checkout-firewall-admin', plugins_url( 'assets/js/checkout-firewall-admin.js', CWF_PLUGIN_FILE ), array(), CWF_VERSION, true );
		if ( 'settings' === $this->view() ) {
			...
			wp_enqueue_script( 'checkout-firewall-recaptcha-admin', plugins_url( 'assets/js/checkout-recaptcha-admin.js', CWF_PLUGIN_FILE ), array(), CWF_VERSION, true );
		}
	}
	...
	public function render(): void {
		...
		$view    = $this->view();
		$status  = isset( $_GET['cwf_status'] ) && is_string( $_GET['cwf_status'] ) ? sanitize_key( wp_unslash( $_GET['cwf_status'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only closed status.
		$license = PremiumAdminPresenterRegistry::license_label();
		?>
	...
	private function overview(): void {
		...
		<section class="cf-protection-hero <?php echo $active ? 'cf-protection-hero--emergency' : ( $observing || ! $recovery_ready ? 'cf-protection-hero--reduced' : '' ); ?>" aria-labelledby="cf-protection-title">
			...
			<div class="cf-protection-hero__actions">
				<?php if ( $observing ) : ?>
					<?php $this->form_open( 'cwf_enable_standard' ); ?><button class="cf-button cf-button--primary" type="submit"><?php esc_html_e( 'Turn on Standard Mode', 'checkout-firewall' ); ?></button></form>
				...
				<?php else : ?>
					<?php $this->form_open( 'cwf_run_health' ); ?><button class="cf-button cf-button--secondary" type="submit"><?php esc_html_e( 'Run a protection check', 'checkout-firewall' ); ?></button></form>
					<?php if ( $active ) : ?>
						<?php $this->form_open( 'cwf_stop_emergency' ); ?><button class="cf-button cf-button--danger" type="submit"><?php esc_html_e( 'End Emergency Mode', 'checkout-firewall' ); ?></button></form>
				<?php else : ?>
					<?php $this->form_open( 'cwf_start_emergency' ); ?><label for="cwf-duration" class="screen-reader-text"><?php esc_html_e( 'Emergency duration', 'checkout-firewall' ); ?></label><select id="cwf-duration" name="duration"><option value="3600"><?php esc_html_e( '1 hour', 'checkout-firewall' ); ?></option><option value="14400" selected><?php esc_html_e( '4 hours', 'checkout-firewall' ); ?></option><option value="43200"><?php esc_html_e( '12 hours', 'checkout-firewall' ); ?></option><option value="86400"><?php esc_html_e( '24 hours', 'checkout-firewall' ); ?></option></select><input type="hidden" name="confirm" value="1" /><button class="cf-button cf-button--danger-outline" type="submit" <?php disabled( ! $recovery_ready ); ?>><?php esc_html_e( 'Turn on Emergency Mode', 'checkout-firewall' ); ?></button></form>
				<?php endif; ?>
				<p><?php esc_html_e( 'Emergency Mode challenges every guest checkout for a fixed period. It never disables a payment gateway.', 'checkout-firewall' ); ?></p>
					...
					if ( ! $active ) :
						?>
						<?php $this->form_open( 'cwf_enable_observe' ); ?><button class="cf-button cf-button--secondary cf-button--small" type="submit"><?php esc_html_e( 'Switch to Observe Mode', 'checkout-firewall' ); ?></button></form><?php endif; ?>
				<?php endif; ?>
			</div>
		</section>

		<?php PremiumAdminPresenterRegistry::render( 'overview' ); ?>
		...
		<section class="cf-section" aria-labelledby="cf-health-title"><div class="cf-section__heading"><h3 id="cf-health-title"><?php esc_html_e( 'Protection health', 'checkout-firewall' ); ?></h3></div><div class="cf-panel"><?php $this->health_rows(); ?><?php $this->form_open( 'cwf_run_health' ); ?><button class="cf-button cf-button--primary" type="submit"><?php esc_html_e( 'Run health check', 'checkout-firewall' ); ?></button></form></div></section>
	...
	private function activity(): void {
		...
		<?php PremiumAdminPresenterRegistry::render( 'activity' ); ?>
...
"><?php esc_html_e( 'Load later expiries', 'checkout-firewall' ); ?></a><?php endif; ?></section>
	...
	?>
		...
		<?php $this->form_open( 'cwf_create_exemption' ); ?><div class="cf-form-grid"><div><label for="cwf-exemption-type"><?php esc_html_e( 'Subject type', 'checkout-firewall' ); ?></label><select id="cwf-exemption-type" name="exemption_type"><option value="ip"><?php esc_html_e( 'IP address or narrow CIDR', 'checkout-firewall' ); ?></option><option value="user"><?php esc_html_e( 'Authenticated WordPress user ID', 'checkout-firewall' ); ?></option></select></div><div><label for="cwf-exemption-value"><?php esc_html_e( 'Value', 'checkout-firewall' ); ?></label><input id="cwf-exemption-value" name="exemption_value" maxlength="254" required autocomplete="off" placeholder="198.51.100.24" /></div><div><label for="cwf-exemption-reason"><?php esc_html_e( 'Reason', 'checkout-firewall' ); ?></label><select id="cwf-exemption-reason" name="exemption_reason"><option value="office_network"><?php esc_html_e( 'Office network', 'checkout-firewall' ); ?></option><option value="wholesale_customer"><?php esc_html_e( 'Wholesale customer', 'checkout-firewall' ); ?></option><option value="vip_customer"><?php esc_html_e( 'VIP customer', 'checkout-firewall' ); ?></option><option value="testing"><?php esc_html_e( 'Authorized testing', 'checkout-firewall' ); ?></option></select></div><div><label for="cwf-exemption-duration"><?php esc_html_e( 'Duration', 'checkout-firewall' ); ?></label><select id="cwf-exemption-duration" name="exemption_duration"><option value="86400"><?php esc_html_e( '1 day', 'checkout-firewall' ); ?></option><option value="604800"><?php esc_html_e( '7 days', 'checkout-firewall' ); ?></option><option value="2592000"><?php esc_html_e( '30 days', 'checkout-firewall' ); ?></option><option value="never"><?php esc_html_e( 'Until removed', 'checkout-firewall' ); ?></option></select></div></div><button class="cf-button cf-button--primary" type="submit"><?php esc_html_e( 'Add trusted exemption', 'checkout-firewall' ); ?></button><p class="cf-help"><?php esc_html_e( 'Email cannot be trusted because a guest can type any billing address. Exact IPs are keyed immediately. CIDRs are stored as entered for range matching and are limited to /24 or narrower for IPv4 and /64 or narrower for IPv6.', 'checkout-firewall' ); ?></p></form></section>
	...
	private function settings(): void {
		...
		<?php PremiumAdminPresenterRegistry::render( 'settings' ); ?>
		<?php $this->challenge_provider_settings(); ?>
		<?php $this->challenge_provider_details(); ?>
		<?php $this->form_open( 'cwf_save_operations' ); ?>
		...
		<details class="cf-panel cf-panel--spaced cf-advanced-details">
			...
			<div class="cf-advanced-details__body" aria-labelledby="cf-proxy-settings">
				...
				<div class="cf-form-grid cf-form-grid--two"><div><label for="cwf-proxy-mode"><?php esc_html_e( 'How traffic reaches WordPress', 'checkout-firewall' ); ?></label><select id="cwf-proxy-mode" name="proxy_mode"><option value="automatic" <?php selected( $proxy, ClientIpResolver::MODE_AUTOMATIC ); ?>><?php esc_html_e( 'Automatic — Direct or verified Cloudflare', 'checkout-firewall' ); ?></option><option value="manual" <?php selected( $proxy, ClientIpResolver::MODE_MANUAL ); ?>><?php esc_html_e( 'Another trusted proxy — host supplied', 'checkout-firewall' ); ?></option></select></div><div><label for="cwf-proxy-header"><?php esc_html_e( 'Visitor-address header — custom proxy only', 'checkout-firewall' ); ?></label><select id="cwf-proxy-header" name="proxy_header"><option value="HTTP_X_FORWARDED_FOR" <?php selected( get_option( ClientIpResolver::HEADER_OPTION, 'HTTP_X_FORWARDED_FOR' ), 'HTTP_X_FORWARDED_FOR' ); ?>>X-Forwarded-For</option><option value="HTTP_X_REAL_IP" <?php selected( get_option( ClientIpResolver::HEADER_OPTION, '' ), 'HTTP_X_REAL_IP' ); ?>>X-Real-IP</option></select></div></div>
				<label for="cwf-proxy-cidrs"><?php esc_html_e( 'Trusted proxy IP ranges — custom proxy only, one CIDR per line', 'checkout-firewall' ); ?></label><textarea id="cwf-proxy-cidrs" name="proxy_cidrs" maxlength="4096" rows="5" placeholder="192.0.2.0/24"><?php echo esc_textarea( is_array( $cidrs ) ? implode( "\n", $cidrs ) : '' ); ?></textarea><p class="cf-help"><?php esc_html_e( 'Cloudflare is detected automatically from its verified network ranges; no Cloudflare setup is needed here. For another proxy, copy the exact header and CIDR ranges supplied by your hosting provider.', 'checkout-firewall' ); ?></p>
			</div>
		</details>
	...
	private function privacy(): void {
		$uninstall_locked = defined( 'CWF_DELETE_DATA_ON_UNINSTALL' ) && true === CWF_DELETE_DATA_ON_UNINSTALL;
		?>
		...
		<?php PremiumAdminPresenterRegistry::render( 'privacy' ); ?>
		<section class="cf-panel cf-panel--spaced" aria-labelledby="cf-licensing-title">
			...
			<p><?php esc_html_e( 'Free protection works locally without an account. Activation and anonymous use create no licensing traffic. Connecting is optional and first shows the Freemius consent screen before licensing and update information is shared.', 'checkout-firewall' ); ?></p>
			...
			<?php if ( CommercialBootstrap::config()->is_configured() ) : ?>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<button class="button cf-button" type="submit"><?php esc_html_e( 'Connect licensing & updates', 'checkout-firewall' ); ?></button>
				</form>
			<?php else : ?>
		...
		<section class="cf-panel cf-panel--spaced" aria-labelledby="cf-uninstall-title">
			...
			<?php $this->form_open( 'cwf_save_uninstall' ); ?>
			...
			<label for="cwf-delete-confirm"><?php esc_html_e( 'Type DELETE when enabling deletion', 'checkout-firewall' ); ?></label><input id="cwf-delete-confirm" type="text" name="confirmation" maxlength="16" autocomplete="off" <?php disabled( $uninstall_locked ); ?> />
			<p class="cf-help"><?php echo $uninstall_locked ? esc_html__( 'CWF_DELETE_DATA_ON_UNINSTALL is enabled in wp-config.php. This control is read-only.', 'checkout-firewall' ) : esc_html__( 'Saving this choice deletes nothing now. Full deletion is irreversible if the plugin is later uninstalled.', 'checkout-firewall' ); ?></p>
	...
	private function challenge_provider_details(): void {
		$selected = $this->challenges->selected();
		?>
		<div id="cwf-provider-local-panel" class="cf-provider-detail" data-cf-provider-panel="local" aria-hidden="<?php echo ChallengeConfig::LOCAL === $selected ? 'false' : 'true'; ?>" <?php echo ChallengeConfig::LOCAL === $selected ? '' : 'hidden'; ?>>
		...
		<div id="cwf-provider-turnstile-panel" class="cf-provider-detail" data-cf-provider-panel="turnstile" aria-hidden="<?php echo ChallengeConfig::TURNSTILE === $selected ? 'false' : 'true'; ?>" <?php echo ChallengeConfig::TURNSTILE === $selected ? '' : 'hidden'; ?>><?php $this->turnstile_settings(); ?></div>
		<div id="cwf-provider-recaptcha-panel" class="cf-provider-detail" data-cf-provider-panel="recaptcha" aria-hidden="<?php echo ChallengeConfig::RECAPTCHA === $selected ? 'false' : 'true'; ?>" <?php echo ChallengeConfig::RECAPTCHA === $selected ? '' : 'hidden'; ?>><?php $this->recaptcha_settings(); ?></div>
		<div id="cwf-provider-none-panel" class="cf-provider-detail" data-cf-provider-panel="none" aria-hidden="<?php echo ChallengeConfig::NONE === $selected ? 'false' : 'true'; ?>" <?php echo ChallengeConfig::NONE === $selected ? '' : 'hidden'; ?>>
	...
	private function recaptcha_settings(): void {
		...
		<section class="cf-panel" aria-labelledby="cf-recaptcha-title">
			...
			<div aria-labelledby="cf-recaptcha-title">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="<?php echo esc_attr( ChallengeSettingsController::SAVE_ACTION ); ?>" /><?php wp_nonce_field( ChallengeSettingsController::NONCE_ACTION ); ?><label for="cwf-recaptcha-site-key"><?php esc_html_e( 'Site key', 'checkout-firewall' ); ?></label><input id="cwf-recaptcha-site-key" name="site_key" type="text" maxlength="128" value="<?php echo esc_attr( $credentials['site_key'] ); ?>" required <?php wp_readonly( defined( 'CWF_RECAPTCHA_SITE_KEY' ) ); ?> /><label for="cwf-recaptcha-secret-key"><?php esc_html_e( 'Secret key', 'checkout-firewall' ); ?></label><input id="cwf-recaptcha-secret-key" name="secret_key" type="password" maxlength="256" value="" autocomplete="new-password" placeholder="<?php echo '' === $credentials['secret_key'] ? esc_attr__( 'Required', 'checkout-firewall' ) : esc_attr__( 'Saved — leave blank to keep it', 'checkout-firewall' ); ?>" <?php wp_readonly( defined( 'CWF_RECAPTCHA_SECRET_KEY' ) ); ?> /><p class="cf-help"><?php esc_html_e( 'Saving a change disables reCAPTCHA until this store verifies the new keys.', 'checkout-firewall' ); ?></p><div class="cf-actions"><button class="button button-primary cf-button" type="submit"><?php esc_html_e( 'Save reCAPTCHA keys', 'checkout-firewall' ); ?></button>
				<?php
	...
	private function turnstile_settings(): void {
		...
		<?php endif; ?>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="<?php echo esc_attr( TurnstileSettingsController::SAVE_ACTION ); ?>" /><?php wp_nonce_field( TurnstileSettingsController::NONCE_ACTION ); ?><label for="cwf-site-key"><?php esc_html_e( 'Site key', 'checkout-firewall' ); ?></label><input id="cwf-site-key" name="site_key" type="text" maxlength="128" value="<?php echo esc_attr( $credentials['site_key'] ); ?>" required <?php wp_readonly( defined( 'CWF_TURNSTILE_SITE_KEY' ) ); ?> /><label for="cwf-secret-key"><?php esc_html_e( 'Secret key', 'checkout-firewall' ); ?></label><input id="cwf-secret-key" name="secret_key" type="password" maxlength="256" value="" autocomplete="new-password" placeholder="<?php echo defined( 'CWF_TURNSTILE_SECRET_KEY' ) ? esc_attr__( 'Configured in wp-config.php', 'checkout-firewall' ) : ( '' === $credentials['secret_key'] ? esc_attr__( 'Required', 'checkout-firewall' ) : esc_attr__( 'Saved — leave blank to keep it', 'checkout-firewall' ) ); ?>" <?php wp_readonly( defined( 'CWF_TURNSTILE_SECRET_KEY' ) ); ?> />
	...
	private function key_settings(): void {
		...
		<details class="cf-panel cf-panel--spaced cf-advanced-details">
			...
			<div class="cf-advanced-details__body" aria-labelledby="cf-key-title">
				...
				<?php $this->form_open( 'cwf_rotate_key' ); ?><label for="cwf-rotate"><?php esc_html_e( 'Type ROTATE to confirm', 'checkout-firewall' ); ?></label><input id="cwf-rotate" type="text" name="confirmation" maxlength="16" autocomplete="off" /><button class="cf-button cf-button--secondary" type="submit" <?php disabled( $this->mode->is_active() ); ?>><?php esc_html_e( 'Rotate identity key', 'checkout-firewall' ); ?></button></form>
			</div>
		</details>
		<?php
	}
	...
	private function view(): string {
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Read-only allowlisted view.
		$view = isset( $_GET['view'] ) && is_string( $_GET['view'] ) ? sanitize_key( wp_unslash( $_GET['view'] ) ) : 'overview';
		// phpcs:enable WordPress.Security.NonceVerification.Recommended
		return in_array( $view, self::VIEWS, true ) ? $view : 'overview'; }
	private function get_date( string $key ): ?string {
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Read-only bounded cursor.
		$value = isset( $_GET[ $key ] ) && is_string( $_GET[ $key ] ) ? wp_unslash( $_GET[ $key ] ) : '';
		// phpcs:enable WordPress.Security.NonceVerification.Recommended
		return 1 === preg_match( '/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/D', $value ) ? $value : null; }
	private function get_id( string $key ): ?int {
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Read-only numeric cursor.
	...
	private function status_copy( string $status ): string {
		$copy    = array(
			...
			'uninstall_constant'            => __( 'Uninstall deletion is controlled by CWF_DELETE_DATA_ON_UNINSTALL.', 'checkout-firewall' ),
		...
		$premium = PremiumAdminPresenterRegistry::status_copy( $status );
		return $copy[ $status ] ?? ( $premium ?? __( 'Settings updated.', 'checkout-firewall' ) ); }
